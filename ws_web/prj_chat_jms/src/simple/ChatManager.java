...
public class ChatManager implements javax.jms.MessageListener {
    ...
    public void open(String dname, String user, String conv, boolean dur, String durname) {
        try {
            ...
            publisher = sus_session.createPublisher(dest);
            // create the receiver (take into account if should be durable)
            subscriber = durable?sus_session.createDurableSubscriber(dest, durablename):sus_session.createSubscriber(dest,"conversacion"+"= '"+conv+"'",false);
    ...
    public void close() {
        try {
            ...
            publisher.close();
            if (durable)
                sus_session.unsubscribe(durablename);
            connection.close();
            this.factory = null;
    ...
    public void send(String text, long timespan) {
        try {
            // EJERCICIO:
            TextMessage m = (TextMessage) sus_session.createTextMessage(text );
            m.setStringProperty("conversacion", convname);
            m.setStringProperty("usuario", username);
            publisher.publish(m,
