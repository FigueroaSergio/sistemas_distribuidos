...
// EJERCICIO: Implemente una clase RectanguloColor, bas�ndose en la clase
// Rectangulo.
...
class RectanguloColor extends Rectangulo {
  ...
  public RectanguloColor(Punto p, int ancho, int alto, int color) {
    super(p.x, p.y, ancho, alto);
    this.color = color;
  }

  @Override
  public String toString() {
    // EJERCICIO: Sobrecargue este m�todo para que incluya en el String tambi�n el
    // color
    return "(Origen: {" + Integer.toString(origen.x) +
        "," + Integer.toString(origen.y) +
        "}, Final: {" + Integer.toString(origen.x + ancho) +
        "," + Integer.toString(origen.y + alto) + "}, Color: " + color + ")";
  }
}
...
public class Objetos {
  ...
  public static void main(String args[]) throws IOException {
    ...
    RC = new RectanguloColor(new Punto(i[0], i[1]), i[2], i[3], 2);// EJERCICIO: Cree una instancia de rectangulo color
                                                                   // RC
    // EJERCICIO: que añada a R3 el atributo de color.

    System.out.println("Nombre de la clase: " + Rectangulo.nombre());
    System.out.println("Nombre de la clase de R1: " + R1.getClass().getName());
    ...
    // EJERCICIO: Cree un CuadradoColor con origen en
