...
public class Message {
	...
	public boolean readFromInputStream(InputStream is) throws IOException {		
		...
		do {
			util.Net.read(is,magicNumerBuff, 1);	
		} while (magicNumerBuff[0] != Message.magicNumber);

		//Read the "payload size" buffer
	    util.Net.read(is,payloadSizeBuff, 4);	
	    int size = util.Bin.dWordToInt(payloadSizeBuff);
	    //Read payload
	    this.payload = new byte[size];
	    util.Net.read(is,this.payload, this.payload.length);
	    //
		return true;
	}
	...
	public boolean writeToOutputStream(OutputStream os) throws IOException {
		...
		byte[] sizeBuff = util.Bin.intToDWord(size);
